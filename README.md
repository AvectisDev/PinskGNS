...
Команда для работы с RFID-считывателями баллонов запускается автоматически при старте приложения через ASGI (см. `GNS/asgi.py`), но также может быть запущена вручную:
...
- Отправка данных о прочитанных метках на сервер через API
