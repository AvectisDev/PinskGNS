...
**Назначение:**
- Обмен данными с постами наполнения через COM (Real COM) или напрямую по TCP с NPort (TCP Server)
...
**Особенности:**
- Чтение 8-байтных кадров, проверка CRC-16
- Атомарная FIFO-очередь Redis `reader_<N>_balloon_queue` (`LPUSH` / `RPOP`)
- Дедупликация повторных запросов в памяти процесса (~2 с)
- Автоматический перезапуск при ошибках транспорта (~60 с) и прочих ошибках (~5 мин)

**Транспорт к постам**

По умолчанию используется COM (`CAROUSEL_<N>_TRANSPORT=com`) — совместимость с Moxa Real COM driver.

Рекомендуемый режим без Windows-драйвера COM: NPort W2150A в **TCP Server**, приложение — TCP-клиент (`TRANSPORT=tcp`). Соединение держится открытым; кадры с RS-485 пушатся в сокет (без polling).

Настройки NPort (web console → Operating Settings / Port):
...
| Packet length | `8` |
| Force transmit | `5–20` ms |
| Inactivity time | `0` (не рвать сессию в паузах) |

После перехода на TCP отключите Real COM mapping на сервере. Не держите одновременно Real COM и TCP-клиент на одном serial-порту NPort.
...
| `CAROUSEL_<N>_TRANSPORT` | транспорт: `com` или `tcp` | `com` |
| `CAROUSEL_<N>_COM_PORT` | COM-порт (при `TRANSPORT=com`) | `COM3` |
| `CAROUSEL_<N>_BAUD_RATE` | скорость COM-порта | `9600` |
| `CAROUSEL_<N>_TCP_HOST` | IP NPort (при `TRANSPORT=tcp`, обязателен) | — |
...
```env
CAROUSEL_1_TRANSPORT=com
CAROUSEL_1_COM_PORT=COM3
CAROUSEL_1_BAUD_RATE=9600
```
