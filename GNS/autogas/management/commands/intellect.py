...
def get_intellect_data(data) -> list:
    """
    посылает запрос в "Интеллект" и возвращает статус запроса и список словарей с записями о транспорте.
...
def get_start_time(delta_minutes: int) -> str:
    """
    определяет время, начиная с которого нужно запросить данные в базе данных "Интеллект".
...
def separation_string_date(date_string: str = '') -> tuple:
    """
    преобразует время, полученное из "Интеллект", в кортеж строк (дата, время) для передачи в JSON
...
def get_registration_number_list(server: dict) -> list:
    """
    формирует тело запроса к базе данных "Интеллект".
...
def check_on_station(transport: dict) -> Optional[bool]:
    """
    обрабатывает направление движения транспорта, определённое "Интеллектом", и возвращает статус
    return:
...
def get_transport_type(registration_number: str) -> str:
    """
    валидирует регистрационный номер транспорта и определяет вид транспорта:
